...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="bg-surface-container-high rounded-3xl shadow-sm border border-outline-variant overflow-hidden">
        ...
        <div class="p-6 lg:p-10 bg-surface">
            ...
            <h1 class="text-3xl lg:text-4xl font-extrabold text-on-surface tracking-tight mb-4">{{ $survey->title }}</h1>
            
            @if($survey->description)
                <p class="text-lg text-on-surface-variant leading-relaxed max-w-4xl mb-10 whitespace-pre-wrap">{{ $survey->description }}</p>
            @else
        ...
        <div class="p-6 lg:p-8">
            ...
            <div x-show="tab === 'pertanyaan'" class="space-y-6">
                @if($survey->questions->count() > 0)
                    @foreach($survey->questions->sortBy('order')->values() as $index => $question)
                        <div class="p-5 border border-outline-variant rounded-xl bg-surface">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 pt-1">
                                    ...
                                    <p class="text-sm text-on-surface-variant mt-1 mb-4">Jenis: {{ $question->question_type == 'essay' ? 'Esai Bebas' : 'Pilihan Ganda' }}</p>
                                    
                                    @if($question->question_type == 'multiple_choice')
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                                            @foreach($question->options()->orderBy('order')->get() as $optIndex => $option)
                                                <div class="flex items-center gap-3 p-3 rounded-xl bg-surface border border-outline-variant hover:border-primary hover:bg-primary/5 transition-all group cursor-default">
                                                    <div class="flex items-center justify-center w-7 h-7 rounded-lg bg-surface-container-high group-hover:bg-primary group-hover:text-primary-content text-on-surface-variant font-bold text-xs transition-colors">
                                                        {{ chr(65 + $optIndex) }}
                                                    </div>
                                                    <span class="text-on-surface text-sm font-medium group-hover:text-primary transition-colors">{{ $option->option_text }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                    ...
                    @foreach($survey->questions->sortBy('order')->values() as $index => $question)
                        <div class="p-6 border border-outline-variant rounded-xl bg-surface">
                            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                                <div>
                                    <h4 class="font-bold text-on-surface text-lg">{{ $index + 1 }}. {{ $question->question_text }}</h4>
                            ...
                            @else
                                <div class="bg-surface-container rounded-xl p-4">
                                    <h5 class="text-sm font-bold text-on-surface-variant uppercase tracking-wider mb-3">Analisis Sentimen (BETA)</h5>
                                    <p class="text-sm text-on-surface mb-3">Dari total jawaban esai yang masuk, berikut adalah perkiraan sentimen secara umum:</p>
                                    
                                    <div class="space-y-3">
                                        <div>
                                            <div class="flex justify-between text-xs font-semibold mb-1">
                                                <span class="text-green-600">Positif</span>
                                                <span class="text-on-surface-variant">TBA%</span>
                                            </div>
                                    ...
                                    <p class="text-xs text-on-surface-variant italic mt-4">*Fitur analisis sentimen ini akan segera aktif di fase pengembangan selanjutnya.</p>
                                </div>
                            @endif
                        </div>
                    @endforeach
                                ...
                                <div class="p-4 flex items-start gap-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between gap-2">
                                            <div class="font-bold text-on-surface truncate">{{ $submission->respondent->name }}</div>
                                        ...
                                        <div class="flex flex-wrap items-center gap-1.5 mt-2">
                                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-surface-container font-semibold text-xs text-on-surface-variant border border-outline-variant/40">
                                                {{ $submission->respondent->gender == 'M' ? 'L' : 'P' }}
                                            </span>
                                            ...
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-secondary/10 text-secondary font-medium text-xs border border-secondary/20">
                                                {{ $submission->respondent->education }}
                                            </span>
                                            <span class="text-xs text-on-surface-variant ml-auto">
                                                {{ $submission->submitted_at ? \Carbon\Carbon::parse($submission->submitted_at)->format('d M Y') : '-' }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <tr>
                                            <td class="py-4 px-6">
                                                <div class="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <div class="font-bold text-on-surface">{{ $submission->respondent->name }}</div>
                                            ...
                                            <td class="py-4 px-6">
                                                ...
                                                <div class="text-xs text-on-surface-variant mt-0.5">
                                                    {{ $submission->submitted_at ? \Carbon\Carbon::parse($submission->submitted_at)->format('H:i') : '-' }} WIB
                                                </div>
                                            </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if($survey->questions->count() > 0 && $survey->submissions_count > 0)
            @foreach($survey->questions()->where('question_type', 'multiple_choice')->get() as $question)
                @php
                    $labels = [];
                    $data = [];
